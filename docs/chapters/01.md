...
在阅读 0 中，我们用 TypeScript 或 Rust 写过同一个小程序：取得内存中的 `status` 和 `body`，解析 `{"text":"你好，Rein"}`，再打印文本。成功和失败都由自己提供的输入决定。
...
本书提供 TS 与 Rust 两种实现路线。本章的请求结构、失败分层和验收思路适用于两种语言，**当前可运行的第 01 章实现位于 `ts/`**，下文命令与源码按它展开。完成[阅读 0 · Rust 版](../readings/00-rust.md)的读者可以先阅读共同流程；Rust 预备练习还不能发送模型请求，正式章节实现仍待建设。

## 从阅读 0 带过来什么 {#from-reading-0}

本章使用 OpenAI 兼容的 Chat Completions 接口，只处理非流式文本回答。接口选择沿用仓库的工程约定；不同端点支持的模型、角色与参数仍需以所用服务的说明为准。
...
如果暂时读不懂某种语法，可以直接查[TS 类型与可选值](../readings/00-ts.md#types)、[JSON 校验](../readings/00-ts.md#json)或[异步与异常](../readings/00-ts.md#async)。终端、HTTP、配置与 Git 见[公共导读](../readings/00.md)。下面集中解释真实调用带来的新问题。

## 先跑起来 {#first-run}

### 确认环境，先完成一次离线检查

如果已经完成 TS 版阅读 0，并且仓库依赖没有变化，可以直接运行下面的测试。首次进入 TS 路线时，按[环境准备](../readings/00-ts.md#setup)安装 Node.js 22 或更高版本，并在仓库根目录执行一次 `npm ci`。根目录的 npm workspace 同时管理文档与 `ts/` 依赖，不需要再进入 `ts/` 安装。
...
这组测试读取仓库里的成功响应和 429 失败响应，不读取 `.env`，也不发送 HTTP 请求。应看到三个用例通过，分别涉及成功解析、HTTP 失败处理和样本脱敏检查。429 用例通过，表示程序正确识别了失败。
...
如果没跑通，先读错误中的类别、变量名或状态码，再对照后面的失败表。不要用反复更换代码来猜配置问题。
...
阅读 0 的请求示意使用顶层 `prompt`；本章所选接口要求把提示词放进消息列表。`model` 指定模型，`messages` 按顺序承载消息。这里仅有一条 `user` 消息。代码还允许传入一条可选的 `system` 消息，放在 `user` 之前；CLI 当前只接收提示词，尚未暴露 system 参数。
...
具体到 Rein，解析器要求 `content` 是字符串；`model` 缺失时显示“未回报”，`finish_reason` 和 `usage` 缺失时保留为 `undefined`，CLI 同样显示“未回报”。这是本章客户端的容错选择，不代表所有提供方的协议都允许省略这些字段。
...
`expectObject()` 在检查后把值表示为 `Record<string, unknown>`，只承诺它是可继续读取的对象，不承诺内部字段合格；后续各层仍然要检查。不能仅凭空数组就判断发生了内容过滤或其他服务端情况。

同样，`content: null` 表示它不满足本章的文本要求，不能单靠这个值断言发生了工具调用。如果响应中同时出现 `tool_calls` 等信息，才有继续检查工具调用的依据。第 03 章会扩展这里的处理范围。
...
当前代码给 429 的是条件式提示，让读者结合响应体判断，没有实现提供方专用的错误解析或恢复策略。
...
后面加入工具调用和循环后，我们可以在这个接口上提供多轮响应，用确定的输入检查执行过程。现在引入它，是为了让调用逻辑从第一章起就可以独立测试。
...
`my-provider` 是示例目录名，请换成描述所用提供方的小写连字符名称。脚本将样本写入 `fixtures/responses/<provider>/hello-<序号>.json`，自动递增序号，不覆盖已有文件。不指定 `--provider` 时，脚本从端点主机名推导名称。
...
| 200，`{"choices":[{"message":{"content":""}}]}` | 返回空文本，元信息显示未回报 | 本章没有继承阅读 0 的非空白限制 |
